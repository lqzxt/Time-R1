...
class RayPPOTrainer(object):
    ...
    def __init__(self,
                 config,
                 tokenizer,
                 role_worker_mapping: dict[Role, WorkerType], # 一个2:actorrollout,一个3:critic,一个 4:refpolicy
        ...
        self.use_reference_policy = Role.RefPolicy in role_worker_mapping #true
        self.use_rm = Role.RewardModel in role_worker_mapping # false
        self.ray_worker_group_cls = ray_worker_group_cls
        self.validation_generations_logger = ValidationGenerationsLogger()

        # define in-reward KL control
        # kl loss control currently not suppoorted
        if config.algorithm.use_kl_in_reward: # false not run this
            self.kl_ctrl_in_reward = core_algos.get_kl_controller(config.algorithm.kl_ctrl)

        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE: # grpo not run this
    ...
    def _validate_config(self):
        ...
        # 1. Check total batch size for data correctness # train_batch_size = 128,actor_rollout_ref.rollout.n = 5
        ...
        if not config.actor_rollout_ref.actor.use_dynamic_bsz: # config.actor_rollout_ref.actor.use_dynamic_bsz = false
            # actor: ppo_micro_batch_size vs. ppo_micro_batch_size_per_gpu run this
            check_mutually_exclusive(config.actor_rollout_ref.actor.ppo_micro_batch_size,
                                     config.actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu,
                                     "actor_rollout_ref.actor")

            if self.use_reference_policy: # run this
        ...
        if self.use_critic and not config.critic.use_dynamic_bsz: # not run this
            # Check for critic micro-batch size conflicts
            check_mutually_exclusive(config.critic.ppo_micro_batch_size, config.critic.ppo_micro_batch_size_per_gpu,
                                     "critic")

        # Check for reward model micro-batch size conflicts
        if config.reward_model.enable and not config.reward_model.use_dynamic_bsz:#enable = false,use_dynamic_bsz = false
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX: # not run this
                    ...
                    if self.config.trainer.balance_batch: # true run this
                    ...
                    # update critic
                    if self.use_critic: # not run this
